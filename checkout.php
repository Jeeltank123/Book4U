...
<body>
  ...
  <div class="col-50">
    ...
    <!-- <input type="text" id="fname" name="firstname" placeholder="Enter your name"  oninput="validateName()"> -->
    <input type="text" id="fname" name="firstname" placeholder="Enter your name" pattern="[A-Za-z]+" title="Only alphabetic characters (A-Z) are allowed" required>
    <span id="nameError" style="color: red; display: none;">Only letters are allowed.</span>

    <label for="email"><i class="fa fa-envelope"></i> Email</label>
    <input type="email" id="email" name="email" placeholder="example@gmail.com" oninput="validateEmail()">
    <span id="emailError" style="color: red; display: none;">Invalid email format. '@' is compulsory.</span>

    <label for="number"><i class="fa fa-phone"></i> Number</label>
    <input type="text" id="number" name="number" placeholder="+910000000000" required pattern="[0-9]{10}">

    <label for="adr"><i class="fa fa-address-card-o"></i> Address</label>
    <input type="text" id="adr" name="address" placeholder="Address">

    <label for="city"><i class="fa fa-institution"></i> City</label>
    <!-- <input type="text" id="city" name="city" placeholder="Your city" oninput="validateText('city')"> -->
    <input type="text" id="city" name="city" placeholder="Your city" pattern="[A-Za-z]+" title="Only alphabetic characters (A-Z) are allowed" required>

    <span id="cityError" style="color: red; display: none;">Only letters are allowed.</span>

    <label for="state"><i class="fa fa-institution"></i> State</label>
    <!-- <input type="text" id="state" name="state" placeholder="State" oninput="validateText('state')"> -->
    <input type="text" id="state" name="state" placeholder="State" pattern="[A-Za-z]+" title="Only alphabetic characters (A-Z) are allowed" required>

    <span id="stateError" style="color: red; display: none;">Only letters are allowed.</span>

    <div style="padding: 0px;" class="row">
      <div class="col-50">
        <label for="country">Country</label>
        <!-- <input type="text" id="country" name="country" placeholder="India" oninput="validateText('country')"> -->
        <input type="text" id="country" name="country" placeholder="country" pattern="[A-Za-z]+" title="Only alphabetic characters (A-Z) are allowed" required>

        <span id="countryError" style="color: red; display: none;">Only letters are allowed.</span>
      </div>
      <div class="col-50">
        <label for="zip">Pincode</label>
        <input type="text" id="zip" name="pincode" placeholder="000000"  required pattern="[0-9]{6}" oninput="validatePincode()">
        <span id="zipError" style="color: red; display: none;">Only numbers are allowed.</span>
      </div>
    </div>
  </div>
</div>
...
<script>  
  function validateName() {
    ...
    nameError.style.display = regex.test(nameInput.value) ? 'none' : 'block';
  }
  ...
  function validatePincode() {
    ...
    zipError.style.display = regex.test(pincodeInput.value) ? 'none' : 'block';
  }
  ...
  function validateText(fieldId) {
    ...
    const regex = /^[a-zA-Z\s]+$/; // Only letters and spaces
    errorField.style.display = regex.test(inputField.value) ? 'none' : 'block';
  }
</script>
